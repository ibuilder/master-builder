...
And, for the Claude Code plugin marketplace:
  plugin/skills/master-builder/   the skill in the layout `/plugin install` expects
                                  (a skills/ dir under the plugin source), generated
                                  from the same source so it cannot drift.
...
def build_plugin_tree() -> int:
    """Mirror the skill into plugin/skills/master-builder/ for the Claude Code marketplace.

    A plugin's skills load from a `skills/` directory under its source, so the marketplace
    entry points at `./plugin`. Keeping that tree under plugin/ (rather than a top-level
    skills/) means cloning this repo straight into ~/.claude/skills/ can't double-load the
    skill. The tree is generated, never hand-edited, and CI fails if it drifts.
    """
    dest = ROOT / "plugin" / "skills" / "master-builder"
    # Remove stale files so a renamed/deleted reference doesn't linger in the plugin copy.
    if dest.exists():
        for old in sorted(dest.rglob("*"), reverse=True):
            if old.is_file():
                old.unlink()
            else:
                old.rmdir()
    ...
    for rel in PACKAGE_FILES:
        target = dest / rel
        target.write_bytes(read(rel).encode("utf-8"))
        written += 1
...
def main() -> int:
    ...
    plugin_files = build_plugin_tree()
    ...
    print(f"  plugin/skills/master-builder/  {plugin_files} files")
